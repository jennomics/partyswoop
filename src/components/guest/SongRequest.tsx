...
export default function SongRequest({ guestCode, onSubmitted }: SongRequestProps) {
  ...
  return (
    <div>
      <h2 className="font-zen font-medium text-h3 text-ink mb-s-3">Request a song</h2>

      {error && <ErrorMessage message={error} onDismiss={() => setError('')} />}

      <form onSubmit={handleSubmit} className="space-y-s-3">
        <div>
          <label className="font-mono text-meta text-ink-50 uppercase block mb-s-1">Song name or artist</label>
          <input
            type="text"
            value={songName}
